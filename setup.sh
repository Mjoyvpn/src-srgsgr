...
BURIQ () {
    curl -sS https://raw.githubusercontent.com/sibeesans/src/main/ip > /root/tmp
...
# https://raw.githubusercontent.com/sibeesans/src/main/ip 
MYIP=$(curl -sS ipv4.icanhazip.com)
Name=$(curl -sS https://raw.githubusercontent.com/sibeesans/src/main/ip | grep $MYIP | awk '{print $2}')
...
PERMISSION () {
    MYIP=$(curl -sS ipv4.icanhazip.com)
    IZIN=$(curl -sS https://raw.githubusercontent.com/sibeesans/src/main/ip | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
    Bloman
    else
    res="Perizinan Diberikan..."
...
if [ "" = "$PKG_OK" ]; then
  ...
  sleep 1
  echo -e "[ ${tyblue}NOTES${NC} ] Otw Menginstal Script"
  echo -e "[ ${tyblue}NOTES${NC} ] Tekan Enter"
  read
else
